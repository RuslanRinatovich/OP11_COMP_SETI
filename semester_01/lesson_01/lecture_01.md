...
Теперь диаграммы должны отображаться корректно, а информация о первой передаче данных стала более наглядной для студентов!
...
```mermaid
flowchart TD
    R[Роутер<br/>(подключен к Интернету)] -->|Wi-Fi| L[Ноутбук]
    R -->|Wi-Fi| P[Телефон]
